...
//	}
	...
	public Object unifiedOrder(@RequestBody WxPayUnifiedOrderRequest request) throws WxPayException, IllegalAccessException {
		...
		// 商品id, 回调查询可以获得
		//request.setProductId("12235413214070356458059");
		res = WxPayConfiguration.getWxPayService(request.getAppid()).unifiedOrder(request);

		// 参数组合
		// 支付签名文档参考
		// https://pay.weixin.qq.com/wiki/doc/api/wxa/wxa_api.php?chapter=7_7&index=3
		AppletPay appletPay = new AppletPay();
		appletPay.setTimeStamp(String.valueOf(Instant.now().getEpochSecond()));
		appletPay.setSignType("MD5");
		appletPay.setRepay_id("prepay_id=" + res.getPrepayId());
		appletPay.setNonceStr(res.getNonceStr());
		appletPay.setPaySign(Signature.getSign(appletPay, request.getAppid()));

		return Lib.GetMapDataSuccess(appletPay);
	}
...
////    request.setBody("优宜定");
////    request.setOutTradeNo(new Date().toString()); // 订单号
////    request.setTotalFee(1);
////    request.setSpbillCreateIp("当前ip");
////    request.setNotifyUrl("通知地址");
////    request.setTradeType("JSAPI");
